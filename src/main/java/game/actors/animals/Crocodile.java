...
public class Crocodile extends Animal {
    ...
    public Crocodile(){
        ...
        addBehaviourToAnimal(new AttackBehaviour(), PRIORITY);
        this.setIntrinsicWeapon(new Teeth());
    ...
    public void spawnCapability(Location spawnGround) {
        ...
        for (Location nearby : spawnGround.getNearbyLocations(DETECT_RADIUS)) {
            if (nearby.containsAnActor()) {
                ...
                nearby.getActor().addStatus(new Poisoning(nearby.getActor(), POISON_DAMAGE,POISON_DURATION));
            }
        }
    }
}
